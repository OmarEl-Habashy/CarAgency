...
if ($conn) {
    ...
    if ($user) {
        $following = $userDAO->getFollowing($user->getUserId());
    } else {
...
<head>
    ...
    <title><?php echo htmlspecialchars($profileUsername); ?> is Following</title>
    <style>
                ...
                <?php foreach ($following as $followedUser): ?>
                    <li class="user-item">
                        <div class="user-avatar">
                            <?php echo strtoupper(substr(htmlspecialchars($followedUser->getUsername()), 0, 1)); ?>
                        </div>
                        <div class="user-info">
                            <a href="../profile.php?username=<?php echo htmlspecialchars($followedUser->getUsername()); ?>" class="username">
                                <?php echo htmlspecialchars($followedUser->getUsername()); ?>
                            </a>
                        </div>
                    </li>
                <?php endforeach; ?>
